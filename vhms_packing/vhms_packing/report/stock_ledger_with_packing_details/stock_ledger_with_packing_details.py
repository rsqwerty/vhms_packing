...
def execute(filters=None):
        ...
        data = update_packing_details(data)
        if filters.ipuid:
            data = get_packing_data(data, "ipuid", filters)

        if filters.puid:
            data = get_packing_data(data, "puid", filters)

        return columns, data

def get_packing_data(data, uom, filters):
    updated_data = []
    for item in data:
        if item[uom] and filters[uom] in item[uom].split("\n"):
            updated_data.append(item)
    return updated_data

def update_packing_details(data):
    for item in data:
        item["ipuid"] = frappe.get_value("Stock Ledger Entry",{"item_code":item["item_code"],"warehouse":item["warehouse"],"voucher_type":item["voucher_type"],"voucher_no":item["voucher_no"]},"ipuid")
        item["puid"] = frappe.get_value("Stock Ledger Entry",{"item_code":item["item_code"],"warehouse":item["warehouse"],"voucher_type":item["voucher_type"],"voucher_no":item["voucher_no"]},"puid")
    return data
...
def get_columns():
        columns = [
                ...
                {"label": _("IPUID"), "fieldname": "ipuid", "width": 110},
                {"label": _("PUID"), "fieldname": "puid", "width": 110},
